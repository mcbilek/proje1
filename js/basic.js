...
function show_questions_for_calis(){
	/*
	fide_all_question();
	if (noq >= 10) {
		for (var i = 0; i < 10; i++) {
...
function show_next_question_for_calis() {
...
function show_question_answer(ilgili_soru,dogrumu,qid,oid,radioId,dogruDiv) {
...
function save_answer_for_calis(qn,oid,dogrumu) {
	...
	$.ajax({
		type : "POST",
		data : str,
		url : base_url + "index.php/quiz/save_answer_for_calis/",
		success : function(data) {
			// signal 1
			$('#save_answer_signal2').css('backgroundColor', '#00ff00');
			setTimeout(function() {
...
function submit_quiz(){
	...
	$('#processing').html("İşleniyor...<br><br>");
	setTimeout(function(){
