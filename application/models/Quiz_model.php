...
function get_qcl($quid){
 ...
 function remove_qid($quid,$qid){
  ...
  function add_qid($quid,$qid){
	 ...
	 //puan sayýsý 1 se zaten hiç soru yok demektir ilk soruyu eklerken dokunmuyoruz
...
if($this->config->item('allow_result_email')){
 ...
 function get_questions_demo_kontrol(){
     ...
     if($logged_in['su']=='0' && $gid!=3){
         //daha önce soru çekilmiþ mi bakýyoruz
 ...
 function get_questions_ders_calis(){
     ...
     if ($cat_id=="-1") {
         ...
         //daha önce tüm sorular çözülmüþ mü diye bakýyoruz
         //çözüldüyse rasgele çözdüðü sorulardan getiriyoruz.
         if ($query->num_rows()==0)
 ...
 function get_questions_oto_deneme($otodeneme_id){
  ...
  //  exit();
         ...
         //kategorilere ait deneme sýnasý soru sayýlarýna göre sorularý çekiyoruz.
             $query=$this->db->query($sql);
             $sonuc = $query->result_array();
             
    //soru sayýsýný update ediyoruz.
