...
Class Qbank_model extends CI_Model
{
 ...
 function soru_onay($qid,$new_status){
     
    return $this->db->query(" update savsoft_qbank set aktifmi='$new_status' where qid='$qid' ");
 ...
 function insert_question_1(){
	 ...
	 foreach($this->input->post('option') as $key => $val){
		 ...
		 if($this->input->post('gerekirse_cikar')==$key){
			 $cikar=1;
		 }else{
			 $cikar=0;
		 }
	...
	$this->db->delete('savsoft_options');
	 foreach($this->input->post('option') as $key => $val){
		 ...
		 if($this->input->post('gerekirse_cikar')==$key)
		     ...
		     $cikar=0;
		     log_message("debug", "gerekise çıkar:".$this->input->post('gerekirse_cikar')); 
		     log_message("debug", "score:".$this->input->post('score')); 
	$userdata=array(
	 'q_option'=>$val,
	 'qid'=>$qid,
	 'score'=>$score,
